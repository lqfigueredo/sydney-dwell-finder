  ...
  .handler(async ({ context }) => {
    ...
    const [listings, wanted, profiles, applications, comments, photos, roles] = await Promise.all([
      ...
      supabase
        .from("profiles")
        .select(
          "id, display_name, suburb, bio, phone, avatar_url, is_business, company_name, deactivated_at, verified_at, verified_until, terms_accepted_at, terms_version, created_at",
        )
        .order("created_at", { ascending: false })
        .limit(500),
      supabase
